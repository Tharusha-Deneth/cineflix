...
const getInitialState = () => {
  ...
  if ((savedUser && loginTime && (new Date().getTime() - parseInt(loginTime, 10) < SEVEN_DAYS)) || isGuest) {
    return 'home'; 
  }
  return 'splash';
};
...
const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      executeSuccessAndGoHome(userData);
      
    } catch (error) {
      console.error("Google Auth Error:", error);
      
      // 🔥 The Magic: Catching AdBlocker / DNS Blocks 🔥
      if (error.code === 'auth/popup-blocked') {
        setErrorMsg("⚠️ Your Private DNS or AdBlocker is blocking Google Login! Please disable it temporarily, OR use Email & Password to Sign Up.");
